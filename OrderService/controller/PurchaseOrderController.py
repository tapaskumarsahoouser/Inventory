...
class CreatePurchaseOrderView(generics.CreateAPIView):
    ...
    #         return renderResponse(data={}, message="User not authenticated", status=401)

    #     print(f"User: {request.user}")  # Debug: Check the user object
    #     print(f"Is authenticated: {request.user.is_authenticated}")  # Debug: Check authentication status
    #     print(f"Domain User ID: {getattr(request.user, 'domain_user_id', None)}")  # Debug: Check domain_user_id
        
    #     domain_user = getattr(request.user, 'domain_user_id', None)
        
    #     if not domain_user or not hasattr(domain_user, 'id'):
    #         return renderResponse(data={}, message="User's domain ID is missing", status=400)
        
    #     po = PurchaseOrder.objects.filter(domain_user_id=domain_user.id, id=id).first() if id else PurchaseOrder()

    #     poFields = getDynamicFormFields(
    #         po, domain_user.id,
    #         skip_related=['supplier_id'],
    #         skip_fields=[
    #             'shipping_cancelled_amount', 'shipping_cancelled_tax_amount', 'approved_at',
    #             'cancelled_at', 'received_at', 'returned_at', 'last_updated_by_user_id',
    #             'status', 'approved_by_user_id', 'cancelled_by_user_id', 'received_by_user_id',
    #             'returned_by_user_id', 'cancelled_reason'
    #         ]
    #     )

    #     return renderResponse(data={'poFields': poFields}, message='Purchase Order Fields', status=200)
