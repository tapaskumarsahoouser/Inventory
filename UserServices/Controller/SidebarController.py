...
#         if request.user.role=='Super Admin' or request.user.domain_user_id==request.user.id:
...
class ModuleView(generics.CreateAPIView):
    def get(self, request):
        ...
        # Serialize the menus and parse JSON
        serialized_menus = json.loads(serialize('json', menus))

        cleaned_menus = []
        for menu in serialized_menus:
            # Extract 'fields' and add the primary key to it
            menu_data = menu['fields']
            menu_data['id'] = menu['pk']

            # Get submenus for each parent menu
            submenus = Modules.objects.filter(
            ...
            # Add submenus to the menu data
            menu_data['submenus'] = list(submenus)
