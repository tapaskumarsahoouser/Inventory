...
from UserServices.models import Modules, UserPermissions, Expoter
...
class ExpoterSerializer(serializers.ModelSerializer):
    class Meta:
        model=Expoter
...
class UserSerializerWithFilters(serializers.ModelSerializer):
    ...
    class Meta:
        model=Expoter
        fields=['id', 'date_joined', 'email', 'phone', 'address', 'city', 'state', 'pincode', 'country', 'profile_pic', 'account_status', 'role', 'dob', 'social_media_links', 'addition_details', 'language', 'departMent', 'designation', 'time_zone', 'last_login', 'last_device', 'last_ip', 'currency', 'domain_name', 'plan_type', 'created_at', 'updated_at', 'domain_user_id', 'added_by_user_id']
...
class UserListView(APIView):
    ...
    def get(self,request):
        users=Expoter.objects.filter(domain_user_id=request.user.domain_user_id.id)
        serializer=ExpoterSerializer(users,many=True)
...
class UserWithFilterListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        queryset=Expoter.objects.filter(domain_user_id=self.request.user.domain_user_id)              #modify by tapas
        return queryset
...
class UpdateUsers(generics.UpdateAPIView):
    ...
    def get_queryset(self):
        return Expoter.objects.filter(domain_user_id=self.request.user.domain_user_id,id=self.kwargs['pk'])                #modify by tapas
