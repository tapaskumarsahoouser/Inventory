...
class SignupAPIView(APIView):
    def post(self,request):
        ...
        user=User.objects.create_user(username=username,email=email,password=password,profile_pic=profile_pic)
        if request.data.get('domain_user_id'):
            user.domain_user_id=User.objects.get(id=request.data.get('domain_user_id'))
        user.save()
