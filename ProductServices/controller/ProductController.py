...
class CreateProductReviewView(generics.CreateAPIView):
    ...
    def perform_create(self,serializer):
        if self.request.data.get('review_user_id'):
            serializer.save(domain_user_id=self.request.user.domain_user_id,review_user_id=User.objects.get(id=int(self.request.data.get('review_user_id'))),product_id=Products.objects.get(id=self.kwargs['product_id']))
        else:
...
class CreateProductQuestionsView(generics.CreateAPIView):
    ...
    def perform_create(self,serializer):
        if self.request.data.get('question_user_id') and self.request.data.get('answer_user_id'):
            serializer.save(domain_user_id=self.request.user.domain_user_id,question_user_id=User.objects.get(id=int(self.request.data.get('question_user_id'))),answer_user_id=Users.objects.get(id=int(self.request.data.get('answer_user_id'))),product_id=Products.objects.get(id=self.kwargs['product_id']))
        else:
    ...
    def perform_update(self,serializer):
        if self.request.data.get('answer'):
            if self.request.data.get('answer_user_id'):
                serializer.save(answer_user_id=User.objects.get(id=int(self.request.data.get('answer_user_id'))))
            else:
